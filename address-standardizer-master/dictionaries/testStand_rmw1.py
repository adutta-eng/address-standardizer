# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 20:04:45 2020

@author: watso330
"""

import standardizer, os
...
adrB_dict = usAdr2Dict(standardizer.standardize(adrStrB))
...
fidAdrA = [x for x in mpStaging.fidOrder]
...
#repeat the whole thing for the seond address (andideally hide the ungly code in a nice function...)
fidB = []
#need to "cheat" and just change a reference list


for col in mpStaging.fidOrder:    
    if col in ('OSN','ZIP'):
        print(col, (adrB_dict[simpleCrossWalk2[col]]))
        fidB.append(adrB_dict[simpleCrossWalk2[col]])
        
    elif col in simpleCrossWalk2.keys():
        #this is... really bad form.  don't ever do this for real.
        try:
            #this is like a master class in bad code, really.  sometimes "clever" is a bad thing            
            fidB.append(stanCode[stanCode2amgScore[simpleCrossWalk2[col]]][adrB_dict[simpleCrossWalk2[col]]])
        except:
            fidB.append(0)
    else:
        #hardcode SNEC...
